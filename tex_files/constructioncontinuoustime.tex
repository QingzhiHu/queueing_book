...
In~\cref{sec:constr-discr-time} we modelled time as progressing in discrete `chunks': minutes, hours, days, and so on.
...
As in a queueing system, jobs can be in queue or in service, and we distinguish between the number in the system $L(t)$, the number in queue $L_Q(t)$, and the number of jobs in service $L_s(t)$.
