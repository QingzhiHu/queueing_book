...
process $\{L_k\}$, and we might try to understand the transient behaviour of the latter by investigating the transient behaviour of the former. For this, we first relate the random walk of the type~\cref{eq:51} to a random walk in continuous time. 
...
The \emph{long-run limiting behaviour} of a queueing system (i.e., the first question) is an important topic by itself.
...
the stationary state will occupy us for the rest of the book.
