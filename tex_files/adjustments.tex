...
For instance, we might decide to make fewer adjustments so that $p$ decreases, but at the expense of larger average outage times.
Now we can analyze the consequences of such decisions without needing actually to do the experiments in real life. 
...
\begin{exercise}\clabel{ex:l-255}
  ...
  Jobs arrive as a Poisson process with the rate $\lambda=9$ per working day.
  The machine works two $8$ hour shifts a day.
  Work not processed on a day is carried over to the next day.
  Job service times are 1.5 hours, on average, with a standard deviation of $0.5$ hours.
